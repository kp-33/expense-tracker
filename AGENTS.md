...
These instructions apply to any AI coding agent working in this repo (Codex, Claude Code, Cursor, etc.). The Claude Code skill at `.claude/skills/expense-import/SKILL.md` defers to this file as the canonical source.

## Project context

`import.py` is a CLI that imports credit card statement CSVs into a Notion expense database. The owner uses it to track personal/family expenses with auto-categorization, deduplication, refund handling, and trip subcategory tagging.

## Expense Import Workflow (required)

When the user shares a CSV path (or several), follow this loop. Do not skip steps and do not push to Notion without explicit user approval.
...
**Categorization**
- Unmapped merchants (`(blank)` in Category column) — propose a `MERCHANT_RULES` addition or one-off override.
- Misclassifications — apply the principles below.
...
- If the user has a recurring legitimate "AUTOPAY" expense getting wrongly skipped, narrow the rule.
